...
DB_USER="$5"
DB_PASS="$6"
TASK_IDENTIFIER="$7"  # TLMView1 or TLMView2 (to identify the task)

TASK_TYPE="TLMView"
TASK_ID="${TASK_IDENTIFIER}"

# Prepare working directory
WORK_DIR="/tmp/Release-Management/${JIRA_TICKET}/deployment/${TASK_TYPE}/${TASK_ID}"
mkdir -p "$WORK_DIR"

# Local path for config-manager.jar
CONFIG_JAR_PATH="/tmp/Release-Management/${JIRA_TICKET}/utilities/${TASK_TYPE}/config-manager.jar"

# Local package path
PACKAGE_PATH="/tmp/Release-Management/${JIRA_TICKET}/client_packages/deploy/${PACKAGE}"
